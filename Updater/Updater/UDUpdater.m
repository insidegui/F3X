...
{
    if ([[NSProcessInfo processInfo].arguments containsObject:@"simulate-update"]) {
        #ifdef DEBUG
        NSLog(@"Updater: simulating update (fake download will take 5 seconds)");
        [self _simulateUpdate];
        return;
        #endif
    }
...
- (void)setDownloadingUpdate:(BOOL)downloadingUpdate
{
    [self willChangeValueForKey:@"isDownloadingUpdate"];
    _downloadingUpdate = downloadingUpdate;
    [self didChangeValueForKey:@"isDownloadingUpdate"];
}
